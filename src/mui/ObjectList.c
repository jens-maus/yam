...
DECLARE(RemoveItem) // Object *item
{
  ...
  if(msg->item != NULL && DoMethod(data->virtgroup, MUIM_Group_InitChange))
  {
    ...
    {
      // tell the application to dispose this object
      DoMethod(_app(obj), MUIM_Application_PushMethod, 2, msg->item, OM_DISPOSE);
    }
