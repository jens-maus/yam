...
DECLARE(Update) // struct Mail *mail
{
  ...
  if(DoMethod(obj, MUIM_Group_InitChange))
  {
    ...
    else if(mail->Folder!= NULL && isOutgoingFolder(mail->Folder) && data->statusImage[SI_WAITSEND] != NULL)
      DoMethod(obj, OM_ADDMEMBER, data->statusImage[SI_WAITSEND]);
    else if(mail->Folder!= NULL && isDraftsFolder(mail->Folder) && data->statusImage[SI_HOLD] != NULL)
