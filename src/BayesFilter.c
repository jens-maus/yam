...
{
    STRPTR tmpContentType, tmpFileName;
            ...
            switch (name[0])
            {
                ...
                case 'r':
                    if(strcmp(name, "received") == 0)
                        if(strstr(content, "may be forged") != NULL)
...
static void tokenAnalyzerSetClassification(struct TokenAnalyzer *ta, struct Tokenizer *t, enum BayesClassification oldClass, enum BayesClassification newClass)
{
    struct TokenEnumeration te;
