...
struct ThreadMessage
{
	...
	BOOL async;             // asynchronous or synchronous execution of function?
...
static struct Thread *StartNewThread(const char *thread_name, int (*entry)(void*), void *eudata)
{
  ...
  ENTER();
	...
	if((thread = (struct Thread*)AllocVec(sizeof(*thread),MEMF_PUBLIC|MEMF_CLEAR)))
	{
		struct ThreadMessage *msg;
...
struct Thread *AddThread(const char *thread_name, int (*entry)(void *), void *eudata)
{
  ...
  ENTER();
	...
	if((thread_node = (struct ThreadNode*)AllocVec(sizeof(struct ThreadNode),MEMF_PUBLIC|MEMF_CLEAR)))
	{
...
int StartAsDefaultThread(int (*entry)(void*), void *eudata)
{
  ...
  {
  	if((default_thread = AddThread("YAM default subthread", entry, eudata)))
	  {
...
static struct ThreadMessage *CreateThreadMessage(void *function, int argcount, va_list argptr)
{
  ...
  ENTER();
	...
	if((tmsg = (struct ThreadMessage *)AllocVec(sizeof(struct ThreadMessage),MEMF_PUBLIC|MEMF_CLEAR)))
	{
...
int CallParentThreadFunctionSync(int *success, void *function, int argcount, ...)
{
  ...
  ENTER();
	...
	if((tmsg = CreateThreadMessage(function, argcount, argptr)))
	{
...
BOOL InitThreads(void)
{
  ...
  ENTER();
	...
	if((G->mainThread.thread_port = CreateMsgPort()))
	{
  ...
  if(IsListEmpty((struct List *)&G->subThreadList) == FALSE)
  {
    ...
    for(curNode = G->subThreadList.mlh_Head; curNode->mln_Succ; curNode = curNode->mln_Succ)
    {
      ...
      // to terminate its jobs...
  		if(timeout == NULL &&
  			 (timeout = (struct TimerMessage*)AllocVecPooled(G->SharedMemPool, sizeof(*timeout))))
      {
      ...
      // request
  		if(mask & timer_m)
  		{
  			...
  			while((timer = (struct TimerMessage*)GetMsg(G->mainThread.timer_port)))
  			{
  				if(timer == timeout)
  				{
      ...
      // process all pending messages on our mainThread message port
  		while((tmsg = (struct ThreadMessage *)GetMsg(G->mainThread.thread_port)))
  		{
