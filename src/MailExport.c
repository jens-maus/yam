...
BOOL ExportMails(const char *fname, const struct MailList *mlist, const BOOL quiet, const BOOL append)
{
  ...
  if((tc.transferGroup = (Object *)DoMethod(G->App, MUIM_YAM_CreateTransferGroup, TR_EXPORT, tc.transferGroupTitle, quiet == FALSE)) != NULL)
  {
    BOOL abort = FALSE;
        ...
        IterateList(&tc.transferList, curNode)
        {
          ...
          if(StartUnpack(mailfile, fullfile, mail->Folder) != NULL)
          {
            ...
            if((mfh = fopen(fullfile, "r")) != NULL)
            {
              ...
              while(xget(tc.transferGroup, MUIA_TransferControlGroup_Aborted) == FALSE &&
          ...
          if(xget(tc.transferGroup, MUIA_TransferControlGroup_Aborted) == TRUE || success == FALSE)
            break;
        }
