...
static int TR_ConnectPOP(int guilevel)
{
  ...
  if(pop3->UseAPOP == TRUE)
  {
    ...
    if((p = strchr(welcomemsg, '<')) != NULL)
    {
      ...
      snprintf(buf, sizeof(buf), "%s %s", pop3->User, digestHex);
      ...
      if(email != NULL)
      {
        ...
        if(rcptok == TRUE)
        {
          ...
          if(TR_SendSMTPCmd(SMTP_DATA, NULL, tr(MSG_ER_BADRESPONSE_SMTP)) != NULL)
          {
            ...
            while(G->TR->Abort == FALSE && G->Error == FALSE &&
                  (curlen = getline(&buf, &buflen, fh)) > 0)
            {
              ...
              if(lineskip == FALSE)
              {
                ...
                if(curlen > 0 && TR_Send(buf, curlen, TCPF_NONE) <= 0)
                {
            ...
            if(G->TR->Abort == FALSE && G->Error == FALSE)
            {
