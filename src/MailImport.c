      ...
      if((tc->importList = CreateMailTransferList()) != NULL)
      {
        ...
        if(IsMailTransferListEmpty(tc->importList) == FALSE)
        {
          ...
          if(isFlagClear(flags, IMPORTF_QUIET) || isFlagSet(flags, IMPORTF_WAIT))
          {
            ...
            if((preselectWin = (Object *)PushMethodOnStackWait(G->App, 5, MUIM_YAMApplication_CreatePreselectionWindow, CurrentThread(), tc->windowTitle, PRESELWINMODE_IMPORT, tc->importList)) != NULL)
            {
              if(SleepThread() == TRUE)
              {
                ULONG result = FALSE;
