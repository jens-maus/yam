...
void rx_mailbounce(UNUSED struct RexxHost *host, void **rxd, enum RexxAction action, UNUSED struct RexxMsg *rexxmsg)
{
  ...
  switch( action )
  {
    case RXIF_INIT:
    {
      if((*rxd = AllocVecPooled(G->SharedMemPool, sizeof(*rd))) != NULL)
        ((struct rxd_mailbounce *)(*rxd))->rc = offsetof(struct rxd_mailbounce, res) / sizeof(long);
    }
    break;
