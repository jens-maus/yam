...
void rx_mailbounce(UNUSED struct RexxHost *host, struct RexxParams *params, enum RexxAction action, UNUSED struct RexxMsg *rexxmsg)
{
  ...
  switch(action)
  {
    ...
    case RXIF_ACTION:
    {
      ...
      if((wmData = NewMessage(NMM_REDIRECT, args->quiet ? NEWF_QUIET : 0L)) != NULL)
      {
