...
void rx_mailedit(UNUSED struct RexxHost *host, struct RexxParams *params, enum RexxAction action, UNUSED struct RexxMsg *rexxmsg)
{
  ...
  switch(action)
  {
    ...
    case RXIF_ACTION:
    {
      ...
      if((wmData = NewMessage(NMM_EDIT, args->quiet ? NEWF_QUIET : 0L)) != NULL)
      {
